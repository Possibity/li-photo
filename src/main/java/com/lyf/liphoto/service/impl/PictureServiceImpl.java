package com.lyf.liphoto.service.impl;
...
import com.lyf.liphoto.model.enums.PictureReviewStatusEnum;
...
import com.lyf.liphoto.mapper.PictureMapper;
...
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture>
    ...
    public QueryWrapper<Picture> getQueryWrapper(PictureQueryRequest pictureQueryRequest) {
        ...
        //JSON数组查询，每个标签都使用 like 模糊查询，将这些条件组合在一起。
    ...
    public Page<PictureVO> getPictureVOPage(Page<Picture> picturePage, HttpServletRequest request) {
        ...
        //若pictureList不空,则要对象列表转化为封装对象列表
        List<PictureVO> pictureVOList=pictureList.stream().map(picture->PictureVO.objToVo(picture)).collect(Collectors.toList());
        //1.关联查询用户信息
        //1,2,3,4把id依次找出来，放在set集合中
        Set<Long> userIdSet=pictureList.stream().map(picture -> picture.getUserId()).collect(Collectors.toSet());
        //1=>user1 2=>user2
